...
{
	unsigned char data_array1[16];//Array of 16 char for Line 1
	unsigned char data_array2[16];//Array of 16 char for Line 2
	...
	for(j=0;j<8;j++)
	{
		...
		if(k>15)//If this condition is true loop arranges data for display onto 2nd line
		{
		 data_array2[k-16]=(temp/100)+48; //Hundred Place
		 temp=temp%100;
		 data_array2[k-15]=(temp/10)+48; //Tens Place
		 temp=temp%10;
		 data_array2[k-14]=(temp)+48; //Unit Place
		 data_array2[k-13]=' ';
		 k=k+4;
		}
		else//Put data on 1st Line
		{
		  data_array1[k]=(temp/100)+48;	//Hundred Place
		  temp=temp%100;
		  data_array1[k+1]=(temp/10)+48; //Tens Place
		  temp=temp%10;
		  data_array1[k+2]=(temp)+48; // Unit Place
		  data_array1[k+3]=' ';
		  k=k+4;
		}
	 }
